...
final class V2ModMetadataParser {
	...
	static LoaderModMetadata parse(JsonReader reader) throws IOException, ParseMetadataException {
		...
		List<V2ModMetadata.MixinEntry> mixins = new ArrayList<>();
		...
		V2ModMetadata.IconEntry icon = null;
		...
		while (reader.hasNext()) {
			...
			case "version":
				...
				break;
			case "provides":
				readProvides(warnings, reader, provides);
				break;
			case "environment":
				if (reader.peek() != JsonToken.STRING) {
		...
		return new V2ModMetadata(id, version, provides,
	...
	private static void readProvides(List<ParseWarning> warnings, JsonReader reader, Map<String, Version> provides) throws IOException, ParseMetadataException {
		...
		while (reader.hasNext()) {
			if (reader.peek() == JsonToken.BEGIN_OBJECT) {
				...
				provides.put(reader.nextString(), null); // null falls back to the mod's version.
			} else {
	...
	private static void readEntrypoints(List<ParseWarning> warnings, JsonReader reader, Map<String, List<EntrypointMetadata>> entrypoints) throws IOException, ParseMetadataException {
		...
		while (reader.hasNext()) {
			...
			while (reader.hasNext()) {
				...
				metadata.add(new V2ModMetadata.EntrypointMetadataImpl(adapter, value));
			}
	...
	private static void readNestedJarEntries(List<ParseWarning> warnings, JsonReader reader, List<NestedJarEntry> jars) throws IOException, ParseMetadataException {
		...
		while (reader.hasNext()) {
			...
			jars.add(new V2ModMetadata.JarEntry(file));
		}

		reader.endArray();
	}

	private static void readMixinConfigs(List<ParseWarning> warnings, JsonReader reader, List<V2ModMetadata.MixinEntry> mixins) throws IOException, ParseMetadataException {
		if (reader.peek() != JsonToken.BEGIN_ARRAY) {
		...
		while (reader.hasNext()) {
			...
			case STRING:
				...
				mixins.add(new V2ModMetadata.MixinEntry(reader.nextString(), ModEnvironment.UNIVERSAL));
				break;
			case BEGIN_OBJECT:
				reader.beginObject();
				...
				while (reader.hasNext()) {
					...
					case "environment":
						environment = V2ModMetadataParser.readEnvironment(reader);
						break;
					case "config":
						if (reader.peek() != JsonToken.STRING) {
				...
				mixins.add(new V2ModMetadata.MixinEntry(config, environment));
				break;
			default:
		...
		while (reader.hasNext()) {
			...
			case BEGIN_OBJECT:
				...
				while (reader.hasNext()) {
					...
					case "contact":
						contactInformation = V2ModMetadataParser.readContactInfo(reader);
						break;
					default:
	...
	private static V2ModMetadata.IconEntry readIcon(JsonReader reader) throws IOException, ParseMetadataException {
		switch (reader.peek()) {
		case STRING:
			return new V2ModMetadata.Single(reader.nextString());
		case BEGIN_OBJECT:
			reader.beginObject();
			...
			return new V2ModMetadata.MapEntry(iconMap);
		default:
